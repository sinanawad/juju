...
- What happens when a CAAS model is migrated between controllers? The deployment type must survive the migration. For inferred types (no explicit constraint), the import side re-infers from charm metadata. For explicit constraints the serialization layer (`description/v12`) must carry the `deployment-type` field through the export/import round-trip; until it does, daemon applications are rejected at the source migration precheck rather than silently converted.
...
**Root cause**: The lifecycle assumes Dying → Dead → DB removal, with K8s cleanup in the Dead phase. But the removal service deletes the DB record as soon as units reach 0, which races with the worker's Dying phase. This race was addressed during the rebase onto upstream/4.0: upstream's removal domain, combined with the `ClearApplicationHasK8sResources` flow, now gates DB removal on K8s cleanup completing, closing the window the original `TODO(k8s)` in this file called out.
