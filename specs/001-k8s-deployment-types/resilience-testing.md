...
**Pass criteria**: Ordinal reset confirms stale sequence cleanup at CAAS
application creation (`deleteApplicationSequence` in `CreateCAASApplication`,
upstream 4.0). The branch previously also deleted the sequence in the removal
domain's `DeleteApplication`; that was dropped as redundant — the create-time
cleanup covers redeploys and, unlike removal-time deletion, does not change
IAAS unit-numbering semantics.
...
3. **Migration may lose deployment type** (MEDIUM): The `description/v12`
   migration serialization does not include the `deployment-type` constraint
   field. Stateful/stateless types are recovered by re-inference on import;
   daemon applications are rejected at the source migration precheck
   (`DaemonDeploymentMigrationNotSupported`) rather than silently converted.
