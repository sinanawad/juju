...
(Superseded during implementation: the inference now runs at deploy time in
the application domain service — `inferDeploymentType` in
`domain/application/service/provider.go` — and the worker reads the persisted
type via `GetApplicationDeploymentType`.)
