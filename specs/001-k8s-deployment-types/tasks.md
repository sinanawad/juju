...
- [x] T001 Add `deployment_type` lookup table and `deployment_type_id` column (`DEFAULT 0`) on the `application` table to the baseline DDL in `domain/schema/model/sql/0019-application.sql`
- ~~T002 Register new PATCH file in `domain/schema/model.go` via `modelPostPatchFilesByVersion`~~ **REMOVED**: the schema change lives in baseline DDL, not a PATCH file, so no separate registration is needed.
...
- [x] T013 [US1+2+4] Implement `inferDeploymentType()` in `domain/application/service/provider.go` — logic: (1) if constraint set → use constraint value, (2) if charm has storage → `stateful`, (3) else → `stateless`. Used by both the deploy path (`CreateCAASApplication`) and the migration import path.
- [x] T014 [US1+2+4] Worker replaces hardcoded `caas.DeploymentStateful` with the persisted deployment type read via `ApplicationService.GetApplicationDeploymentType()` in `internal/worker/caasapplicationprovisioner/application.go:158`
...
- [x] T020 [US1+2+4] Add warning log when `deployment-type=stateless` but charm declares persistent storage, emitted in `appAlive()` in `internal/worker/caasapplicationprovisioner/ops.go`
...
- [x] T022 [US3] Add deployment type check in scale validation in `domain/application/service/application.go` — before processing scale change, query application deployment type; if `daemon`, return `DaemonSetScaleNotSupported` error. Implemented in `Service.SetApplicationScale`/`ChangeApplicationScale` with tests.
...
- [ ] T042 [Migration] Update the constraints export path to include `DeploymentType` field once `description` supports it (location TBD — `exportApplicationConstraints()` in `domain/application/modelmigration/export.go` does not exist in the current codebase; locate the equivalent export function once T040/T041 land)
...
- [ ] T055 [US1+2+4] Add unit tests for `RegisterCAASUnit` with `OrderedScale=false` in `domain/application/state/state_test.go` — verify Deployment/DaemonSet units can register while `aliveCount < appScale.Scale` even when `Scaling=false`; verify rejection when `aliveCount >= appScale.Scale` (gate counts alive units via `countAliveUnitsForApplication`, not ordinals)
...
- [x] T049 [US1+2+4] Modify `RegisterCAASUnit` scaling gate in `domain/application/state/unit.go` to differentiate by `OrderedScale` flag: StatefulSet requires `appScale.Scaling == true` AND `ordinal < ScaleTarget` (strict gate); Deployment/DaemonSet counts alive units via `countAliveUnitsForApplication` and rejects when `aliveCount >= appScale.Scale` (relaxed gate — ordinals are not consulted; pods start before `EnsureScale` sets `Scaling=true`)
