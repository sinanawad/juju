...
domain/schema/model/sql/        # deployment_type lookup table + column (baseline DDL in 0019-application.sql)
...
4. Add `deployment_type` lookup table + `deployment_type_id` column (`DEFAULT 0`) on `application` to the baseline DDL in `domain/schema/model/sql/0019-application.sql`
5. Add `DeploymentType` field to `AddCAASApplicationArg` in `domain/application/types.go`
6. Persist deployment type in `domain/application/state/application.go` during CreateCAASApplication
7. Add `GetApplicationDeploymentType()` to application service
8. Add `DeploymentType` to `params.CAASApplicationProvisioningInfo`
9. Bump CAASApplicationProvisioner facade to v2; populate deployment type in provisioning info
10. Update `api/facadeversions.go` with new version
11. Infer deployment type in `inferDeploymentType()` (`domain/application/service/provider.go`) at `CreateCAASApplication` time: constraint → storage heuristic → default
12. Replace hardcoded `caas.DeploymentStateful` in `application.go:149` with dynamic type
13. Replace hardcoded `caas.DeploymentStateful` in `caasfirewaller/appfirewaller.go:81`
14. Replace hardcoded `caas.DeploymentStateful` in `domain/application/service/provider.go:442,538`
15. Add warning when `deployment-type=stateless` but charm has storage (FR-012)
16. Add `DeploymentTypeImmutable` error and enforce immutability in `SetApplicationConstraints` (FR-006)
...
### RELEASE BLOCKER: `description/v12` — Add `DeploymentType` to Constraints

The `github.com/juju/description/v12` library serializes model data during migration. Its `ConstraintsArgs` struct and `Constraints` interface do **not** include a `DeploymentType` field. This means:
...
**Mitigation in place**: Import re-infers deployment type from charm metadata when no constraint is available. This correctly handles the common case (inferred stateless/stateful) but cannot recover explicit overrides or `daemon`. As an interim mitigation, `CheckApplicationsForMigration` rejects daemon applications at the migration precheck with `DaemonDeploymentMigrationNotSupported`, so daemon apps fail fast instead of silently changing type on migration; stateful and stateless applications survive migration via re-inference.
...
2. Bump to the `description` release containing `DeploymentType` in `go.mod`
3. Update the constraints export path to include `DeploymentType` (location TBD after upstream description support lands)
...
- `domain/schema/model/sql/0019-application.sql` — deployment_type lookup table + deployment_type_id column
...
- `internal/worker/caasapplicationprovisioner/ops.go` — deployment type threading through `EnsureScale`/`updateState`, stale-pod reconciliation gated to non-stateful deployment types
