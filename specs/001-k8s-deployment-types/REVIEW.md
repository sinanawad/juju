...
The `description/v12` library lacks `DeploymentType` support, so explicit
...
- `deployment_type` lookup table + FK on `application` (baseline DDL in `domain/schema/model/sql/0019-application.sql`)
...
- Worker: reads persisted deployment type via `GetApplicationDeploymentType`; inference happens at deploy time in the application domain service
...
- `updateState()` tolerates `applicationerrors.UnitNotFound` from `ClearCAASUnitCloudContainer` (prevents crash when the removal service races ahead)
...
Commit subjects (hashes change on rebase; see `git log upstream/4.0..001-k8s-deployment-types`):

```
docs: add speckit tooling and k8s deployment type design artifacts
docs: fix analysis gaps in spec, plan, and tasks
feat: add deployment-type constraint for K8s workload selection (MVP)
feat: fix review issues, add non-StatefulSet unit registration and provider support
feat: add pod recovery for Deployment/DaemonSet and fix unit ordinal inflation
docs: add resilience testing plan, script, and results
docs: comprehensive REVIEW.md for MVP staff review
docs: add storage adaptation stories and fix spec consistency issues
fix: resolve Deployment scale-down deadlock in CAAS provisioner
docs: expand resilience test plan to 42 scenarios across 10 groups
docs: add consolidated Juju knowledge base for team reference
```
...
| `domain/schema/model/sql/0019-application.sql` | Lookup table + FK (baseline DDL) |
...
| `apiserver/facades/controller/caasapplicationprovisioner/service.go` | `GetApplicationDeploymentType` added to `ApplicationService` interface |
...
| `internal/worker/caasapplicationprovisioner/ops.go` | deployment type threading through `EnsureScale`/`updateState`, stale pod reconciliation |
...
**Superseded by upstream 4.0 rebase**: Two fixes from the original branch no longer apply after rebasing onto upstream/4.0. The removal-domain sequence deletion (`application_scale` before `unit_agent_status`) was dropped because upstream now deletes at CAAS creation. The `appDying()` `UnitNotFound` tolerance was superseded by upstream's removal domain flow — the remaining tolerance is `updateState()` ignoring `UnitNotFound` from `ClearCAASUnitCloudContainer`.

---

## One Question for Reviewer

The migration story has a hard dependency on the external `description/v12`
