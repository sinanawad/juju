...
**Status**: Phases 1-3 complete (MVP), Phases 3a/3b complete, Phase 7 pod recovery complete, Phase 4 (daemon scale guard) NOW IMPLEMENTED (`Service.SetApplicationScale`/`ChangeApplicationScale` reject daemon via `DaemonSetScaleNotSupported`), Phase 6 partial (daemon migration guarded by precheck rejection; full fix awaits description support), Phase 5 (status visibility) pending
...
- Schema: `deployment_type` lookup table + column on `application` (baseline DDL in `domain/schema/model/sql/0019-application.sql`)
...
- Worker: reads persisted deployment type from the application service; inference at deploy time in `domain/application/service/provider.go`
...
### Phase 4: DaemonSet Scale Blocking (T021-T022) — IMPLEMENTED
- `DaemonSetScaleNotSupported` error constant
- Check in `Service.SetApplicationScale()` / `ChangeApplicationScale()` in `domain/application/service/application.go`, with tests
...
- `description/v12` lacks `DeploymentType` in `ConstraintsArgs`/`Constraints`
...
- **Interim mitigation**: daemon apps are rejected at migration precheck (`DaemonDeploymentMigrationNotSupported`)
- See plan.md "Pre-Merge Dependencies" section

### Cross-Cutting Tests (T030-T035)
...
### Assessment
These gaps are **pre-existing** in the K8s provider and affect all deployment types equally. `computeStatus()` and `currentScale()` are now implemented for all three deployment types (Deployment, StatefulSet, DaemonSet). The remaining gaps are low-severity and not a blocker for this feature.
...
| Schema DDL | `domain/schema/model/sql/0019-application.sql` |
