...
// If the deployment-type constraint differs from the persisted value, an
// error satisfying [applicationerrors.DeploymentTypeImmutable] is returned.
	...
	if cons.HasDeploymentType() {
		...
		if *cons.DeploymentType != currentDT {
			return errors.Errorf(
				"application %q is %q; redeploy to use a different workload type: %w",
				appDetails.Name, currentDT, applicationerrors.DeploymentTypeImmutable,
			)
		}
	}
...
func (s *ProviderService) RegisterCAASUnit(
	...
	s.logger.Debugf(ctx, "RegisterCAASUnit: app=%q providerID=%q deploymentType=%q",
		appName, params.ProviderID, deploymentTypeStr)
	...
	if deploymentTypeStr == string(caas.DeploymentStateful) {
		...
		if found {
			...
			if foundUnassigned {
				...
				ord, err = s.st.GetNextCAASUnitOrdinal(ctx, appUUID)
				if err != nil {
	...
	registerArgs.OrderedScale = deploymentTypeStr == string(caas.DeploymentStateful)
...
// inferDeploymentType returns the deployment type for a CAAS application.
// An explicit constraint always wins; otherwise charms that declare storage
// default to stateful and charms without storage to stateless.
func inferDeploymentType(constraint *string, hasStorage bool) string {
	if constraint != nil && *constraint != "" {
		return *constraint
	}
	if hasStorage {
		return string(caas.DeploymentStateful)
	}
	return string(caas.DeploymentStateless)
}
...
func (s *ProviderService) makeCAASApplicationArg(
	...
	addCAASApplicationArg := application.AddCAASApplicationArg{
		BaseAddApplicationArg: arg,
		Scale:                 len(units),
		DeploymentType:        inferDeploymentType(cons.DeploymentType, len(charm.Meta().Storage) > 0),
	}
