...
func (s *unitStateSuite) TestGetNextCAASUnitOrdinalCountsDeadUnits(c *tc.C) {
	...
	for i, name := range []coreunit.Name{"bar/0", "bar/1"} {
		p := application.RegisterCAASUnitArg{
			...
			Address:      new(fmt.Sprintf("10.0.0.%d/8", i)),
	...
	ord, err := s.state.GetNextCAASUnitOrdinal(c.Context(), appUUID)
	c.Assert(err, tc.ErrorIsNil)
	...
	// Dead units still occupy their names until reaped, so the next
	// ordinal must clear them rather than reusing an occupied name.
	ord, err = s.state.GetNextCAASUnitOrdinal(c.Context(), appUUID)
	c.Assert(err, tc.ErrorIsNil)
	c.Assert(ord, tc.Equals, 2)
}
...
func (s *unitStateSuite) TestGetNextCAASUnitOrdinalIncludesDyingUnits(c *tc.C) {
	appUUID := s.createCAASScalingApplication(c, "baz", life.Alive, 2)
	...
	for i, name := range []coreunit.Name{"baz/0", "baz/1"} {
		p := application.RegisterCAASUnitArg{
			...
			Address:      new(fmt.Sprintf("10.1.0.%d/8", i)),
			Ports:        new([]string{"9090"}),
			OrderedScale: true,
			OrderedId:    i,
		}
