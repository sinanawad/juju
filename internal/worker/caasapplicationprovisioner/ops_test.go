...
func (s *OpsSuite) TestUpdateState(c *tc.C) {
	...
	currentReportedStatus, err := caasapplicationprovisioner.AppOps.UpdateState(c.Context(), "test", appId, app, lastReportedStatus,
		string(caas.DeploymentStateless), broker, applicationService, statusService, clk, s.logger)
	c.Assert(err, tc.ErrorIsNil)
...
func (s *OpsSuite) TestUpdateStateClearsStalePods(c *tc.C) {
	...
	_, err := caasapplicationprovisioner.AppOps.UpdateState(
		c.Context(), "test", appId, app, nil, string(caas.DeploymentStateless),
		broker, applicationService, statusService, clk, s.logger,
	)
	c.Assert(err, tc.ErrorIsNil)
}
